...
class WacomDrawingCanvas(QWidget):
    def __init__(self, ink_system, config: ProcessingConfig):
        ...
        # 🆕 獲取螢幕資訊（雙螢幕支援）
        self.primary_screen, self.secondary_screen = self._detect_screens()
    ...
    def _detect_screens(self):
        ...
        if screen_count > 1:
            ...
            self.logger.info("✅ 將使用雙螢幕模式：對話框在主螢幕，畫布在副螢幕")
        else:
    ...
    def _setup_screen_size(self):
        ...
        self.logger.info(f"📐 畫布尺寸（副螢幕）: {canvas_width} x {canvas_height}")
