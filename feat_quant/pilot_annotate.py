...
class AnnotationWindow(QMainWindow):
    ...
    def on_next_clicked(self):
        ...
        # 🆕🆕🆕 計算標準化座標（normalized to 0-1）
        x_start_norm = bbox_info['x'] / self.canvas_width
        x_end_norm = (bbox_info['x'] + bbox_info['width']) / self.canvas_width
        x_range_norm = bbox_info['width'] / self.canvas_width
        
        y_start_norm = bbox_info['y'] / self.canvas_height
        y_end_norm = (bbox_info['y'] + bbox_info['height']) / self.canvas_height
        y_range_norm = bbox_info['height'] / self.canvas_height
        
        result = {
            'subject_id': self.current_subject_id,
            'drawing_id': self.current_drawing_id,
    ...
    def _export_excel(self, output_path, result):
        ...
        data = {
            ...
            '數值': [
                ...
                result['drawing_id'],
                result['folder_name'],
                f"{result['x_start_norm']:.4f}",  # 🆕 標準化到 0-1
                f"{result['x_end_norm']:.4f}",    # 🆕 標準化到 0-1
                f"{result['x_range_norm']:.4f}",  # 🆕 標準化到 0-1
                f"{result['y_start_norm']:.4f}",  # 🆕 標準化到 0-1
                f"{result['y_end_norm']:.4f}",    # 🆕 標準化到 0-1
                f"{result['y_range_norm']:.4f}",  # 🆕 標準化到 0-1
                result['canvas_width'],            # 原始像素值
                result['canvas_height']            # 原始像素值
            ]
        }
    ...
    def _generate_histograms(self, df, output_dir):
        """生成 histogram（🆕 標準化座標版本）"""
        features = [
            ('x_start_norm', 'X Start (Normalized)'),
            ('x_end_norm', 'X End (Normalized)'),
            ('x_range_norm', 'X Range (Normalized)'),
            ('y_start_norm', 'Y Start (Normalized)'),
            ('y_end_norm', 'Y End (Normalized)'),
            ('y_range_norm', 'Y Range (Normalized)')
        ]
